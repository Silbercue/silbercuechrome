 ...
 * Usage: node test-hardest/benchmark-full.mjs [--operator-mode]
 * Requires: Chrome on port 9222, benchmark server on port 4242
 * Outputs: JSON benchmark results to file
 *
 * --operator-mode: Runs the benchmark with operator card-recognition tracking.
 * Before each test, calls the `operator` tool (scan-phase) to detect card matches
 * on the current page. Tests execute via primitive tools for deterministic results.
 * Writes extended JSON output with:
 *   - recognition_rate, false_positive_rate, mqs, wall_clock_ms, operator_mode, production_validation
 *
...
/** When true, benchmark uses only `operator` + `virtual_desk` tools. */
const OPERATOR_MODE = process.argv.includes("--operator-mode");
...
/** Tracks which tests had at least one card match (operator-mode only). */
let testsWithCardMatch = 0;
/** Tracks card matches that led to follow-up errors (false positives). */
let cardMatchErrors = 0;
/** Tracks total card matches across all tests. */
let totalCardMatches = 0;
...
/**
 * Operator-mode scan: calls `operator` tool (scan-phase, no card/params)
 * to detect card matches on the current page. Updates tracking counters.
 * Returns true if at least one card matched.
 */
async function operatorScan(client) {
  if (!OPERATOR_MODE) return false;
  try {
    const res = await client.callTool({ name: "operator", arguments: {} });
    const text = res.content
      ?.filter((c) => c.type === "text")
      .map((c) => c.text)
      .join("\n") ?? "";
    // Detect card match from operator response — matched cards appear as
    // "MATCHED CARD:" or "card:" in the offer text, or contain "score:" lines.
    // A fallback/no-match response contains "no card" or "fallback".
    const hasMatch = /MATCHED CARD|card:|score:\s*0\.[5-9]|score:\s*1\./i.test(text)
      && !/no card|fallback.*mode|kein.*karte/i.test(text);
    if (hasMatch) {
      totalCardMatches++;
      return true;
    }
  } catch {
    // Operator tool not available or error — don't break the benchmark
  }
  return false;
}
...
async function runTest(id, name, client, fn) {
  ...
  // Operator-mode: scan for card matches before executing the test
  const hadCardMatch = await operatorScan(client);
  if (hadCardMatch) testsWithCardMatch++;
  ...
  try {
    ...
    // If this test had a card match but failed, it may be a false positive
    if (hadCardMatch) cardMatchErrors++;
  }
}

// ── Main ──
const modeLabel = OPERATOR_MODE ? " [OPERATOR MODE]" : "";
console.error(`\n${BOLD}SilbercueChrome Full Benchmark (31 Tests)${modeLabel}${RESET}\n`);
...
const transport = new StdioClientTransport({
  ...
  env: {
    ...process.env,
    // Operator-mode needs the full tool-set (includes operator + virtual_desk)
    ...(OPERATOR_MODE && { SILBERCUE_CHROME_FULL_TOOLS: "true" }),
  },
});
...
const wallClockMs = benchmarkEnd - benchmarkStart;

// Operator-mode metrics (Subtask 1.6, 1.7, 1.8, 1.9)
const recognitionRate = totalTests > 0 ? testsWithCardMatch / totalTests : 0;
const falsePositiveRate = totalCardMatches > 0 ? cardMatchErrors / totalCardMatches : 0;

// MQS calculation aligned with MQS-Framework (project_mqs-framework.md):
// MQS = PassRate * 0.4 + ToolEfficiency * 0.3 + TokenEfficiency * 0.3
// PassRate = passed/total (0-1)
// ToolEfficiency: inverse of avg tool calls per passed test (lower is better)
// TokenEfficiency: approximated from total tool calls vs theoretical minimum
const passRate = totalTests > 0 ? passed / totalTests : 0;
const avgCallsPerTest = passed > 0 ? totalCalls / passed : totalCalls;
// Ideal: ~2 calls/test. Efficiency drops linearly; floor at 0.
const toolEfficiency = Math.max(0, Math.min(1, 2 / Math.max(avgCallsPerTest, 1)));
// Token efficiency: approximated — fewer total calls = better
const theoreticalMinCalls = totalTests * 1.5; // ~1.5 calls per test as ideal
const tokenEfficiency = Math.max(0, Math.min(1, theoreticalMinCalls / Math.max(totalCalls, 1)));
const mqs = Math.round(
  (passRate * 0.4 + toolEfficiency * 0.3 + tokenEfficiency * 0.3) * 100
);

if (OPERATOR_MODE) {
  console.error(`\n${BOLD}Operator-Mode Metrics:${RESET}`);
  console.error(`  recognition_rate     : ${(recognitionRate * 100).toFixed(1)}% (${testsWithCardMatch}/${totalTests})`);
  console.error(`  false_positive_rate  : ${(falsePositiveRate * 100).toFixed(1)}% (${cardMatchErrors}/${totalCardMatches})`);
  console.error(`  mqs                  : ${mqs}`);
  console.error(`  wall_clock_ms        : ${wallClockMs}`);
}
...
const output = {
  ...
  notes: OPERATOR_MODE
    ? "Automated benchmark via benchmark-full.mjs --operator-mode. Card recognition tracked via operator scan."
    : "Automated benchmark via npm run benchmark. 31 tests across 5 levels.",
  summary: {
    total: totalTests,
    passed,
    failed,
    ...
    total_time_ms: wallClockMs,
    tool_uses: totalCalls,
    total_tool_calls: totalCalls,
  },
  tests: testResults,
  // Operator-mode extended fields (AC-6)
  ...(OPERATOR_MODE && {
    operator_mode: true,
    ...
    // AC-3: production_validation placeholder — populated by validate-card.ts pipeline
    production_validation: {
      status: "pending",
      detail: "Production validation runs via validate-card.ts, not inline in benchmark",
    },
  }),
};

const today = new Date().toISOString().slice(0, 10);
const suffix = OPERATOR_MODE ? "-operator" : "";
const outPath = new URL(`benchmark-silbercuechrome_mcp${suffix}-${today}.json`, import.meta.url).pathname;
